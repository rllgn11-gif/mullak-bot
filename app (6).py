...
@rate_limit(5, 3600)   # max 5 اقتراحات في الساعة
@require_auth
def submit_suggestion(user):
    try:
        body     = request.get_json(silent=True) or {}
        category = esc(body.get("category", "أخرى"))[:50]
        text     = esc(body.get("text", "")).strip()

        if not text:
            return jsonify({"error": "نص الاقتراح مطلوب"}), 400
        if len(text) > 1000:
            return jsonify({"error": "الاقتراح طويل جداً (الحد 1000 حرف)"}), 400

        fname   = esc(user.get("first_name", "مجهول"))
        user_id = user.get("user_id", "?")

        # إرسال للأدمن عبر تيليجرام
        if bot and ADMIN_ID:
            msg = (
                f"💡 *اقتراح جديد من إدارة العقارات*\n"
                f"━━━━━━━━━━━━━━━━━\n"
                f"👤 المستخدم: *{fname}* (`{user_id}`)\n"
                f"🏷️ النوع: {category}\n"
                f"━━━━━━━━━━━━━━━━━\n"
                f"📝 {text}"
            )
            try:
                bot.send_message(int(ADMIN_ID), msg, parse_mode="Markdown")
            except Exception as e:
                print(f"❌ إرسال الاقتراح للأدمن: {e}")
