...
def sb_update(table, filters, data):
    r = requests.patch(f"{SUPABASE_URL}/rest/v1/{table}", headers=sb_headers(), params=filters, json=data, timeout=10)
    r.raise_for_status()
    result = r.json()
    return result[0] if isinstance(result, list) and result else {"ok": True}

def sb_delete(table, filters):
    r = requests.delete(f"{SUPABASE_URL}/rest/v1/{table}", headers=sb_headers(), params=filters, timeout=10)
    r.raise_for_status()
    return {"ok": True}
...
def add_property(user):
    try:
        ...
        result = sb_insert("properties", {
            ...
            "investor_rent": int(d.get("investor_rent", 0) or 0),
            "contract_desc": esc(d.get("contract_desc", ""))
        })
...
def add_tenant(user):
    try:
        ...
        prop_id_v = d.get("property_id")
        unit_num_v = d.get("unit_num")
        if prop_id_v and unit_num_v:
            sb_update("units",
                {"property_id": f"eq.{prop_id_v}",
                 "unit_num":    f"eq.{unit_num_v}",
                 "user_id":     f"eq.{user['user_id']}"},
                {"tenant_name": d.get("name", "")})
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_today(user):
    try:
        ...
        units    = sb_select("units",      {"user_id": f"eq.{uid}"})
        expenses = sb_select("expenses",   {"user_id": f"eq.{uid}"}, order="created_at.desc")

        # من يدفع اليوم (عقد ينتهي اليوم)
        ending_today = [t for t in tenants if t.get("end_date") == today]

        # عقود تنتهي خلال 7 أيام
        from datetime import timedelta
        week_later = (datetime.now().date() + timedelta(days=7)).isoformat()
        expiring_soon = [t for t in tenants
                         if t.get("end_date") and today < t["end_date"] <= week_later]

        # المتأخرون (لم يدفعوا)
        unpaid = [t for t in tenants if not t.get("paid")]

        # الوحدات الفارغة
        vacant_units = [u for u in units if not u.get("tenant_name")]

        # ملخص مالي
        income_today  = sum(t["rent"] for t in ending_today if t.get("paid"))
        total_unpaid  = sum(t["rent"] for t in unpaid)
        total_income  = sum(t["rent"] for t in tenants if t.get("paid"))
        total_exp     = sum(e.get("amount", 0) for e in expenses)

        # إجراءات سريعة مقترحة
        actions = []
        if unpaid:
            actions.append({"icon": "💳", "label": f"تسجيل دفعة ({len(unpaid)} متأخر)", "screen": "paid"})
        if vacant_units:
            actions.append({"icon": "🚪", "label": f"وحدة فارغة ({len(vacant_units)})", "screen": "addTenant"})
        actions.append({"icon": "📤", "label": "إضافة مصروف", "screen": "addExpense"})

        def safe_tenant(t):
            return {
                "id":         t.get("id"),
                "name":       t.get("name", ""),
                "unit_num":   t.get("unit_num"),
                "rent":       t.get("rent", 0),
                "paid":       t.get("paid", False),
                "end_date":   t.get("end_date"),
                "phone":      t.get("phone", ""),
                "prop_name":  (t.get("properties") or {}).get("name", "") if isinstance(t.get("properties"), dict) else ""
            }

        return jsonify({
            "ending_today":   [safe_tenant(t) for t in ending_today],
            "expiring_soon":  [safe_tenant(t) for t in expiring_soon],
            "unpaid":         [safe_tenant(t) for t in unpaid],
...
def edit_expense(user, exp_id):
    try:
        d       = request.json or {}
        allowed = ["category", "description", "amount"]
        updates = {k: d[k] for k in allowed if k in d}
        if "description" in updates:
            updates["description"] = esc(updates["description"])
...
# ============================================================
# 🧠 Smart Input Helpers + Routes
# ============================================================
AR_NUMS = {'واحد':1,'واحدة':1,'اثنين':2,'اثنان':2,'ثنين':2,'ثنتين':2,'اثنتين':2,'ثلاث':3,'ثلاثة':3,'أربعة':4,'اربعة':4,'خمسة':5,'ستة':6,'سبعة':7,'ثمانية':8,'تسعة':9,'عشرة':10}
...
def ar_num(val):
    ...
    return AR_NUMS.get(s)

def calc_end_date(start_date:str, period:str, count:int):
    try:
    ...
    if period == 'يوم':
        end = dt + timedelta(days=count-1)
    elif period == 'شهر':
        end = dt + timedelta(days=30*count)
    else:
        end = dt + timedelta(days=365*count)
    return end.date().isoformat()

def find_property_for_text(user_id:str, text:str):
    props = sb_select('properties', {'user_id': f'eq.{user_id}'})
    active = [p for p in props if p.get('type') != 'محذوف']
    if not active: return None
    nt = normalize_text(text)
    for p in active:
        if normalize_text(p.get('name','')) in nt and p.get('name'): return p
    if len(active) == 1: return active[0]
    return None

def parse_property_text(text:str):
    raw = text.strip()
    nt = normalize_text(raw)
    ptype = 'مستثمر' if any(w in nt for w in ['مستثمر','استثمار','بالباطن']) else 'مالك'
    location = ''
    m = re.search(r'(?:في|بحي|بالرياض|بجده|بمكه|بالمدينه)\s+([^،,.]+)', raw)
    if m: location = m.group(1).strip()
    investor_rent = 0
    m = re.search(r'(\d+)\s*ريال', raw)
    if m and ptype == 'مستثمر': investor_rent = int(m.group(1))
    type_word = ''
    for w in ['عمارة','عماره','برج','فيلا','فله','شاليه','استراحة','استراحه','مجمع']:
        if normalize_text(w) in nt:
            type_word = w
            break
    name = ''
    m = re.search(r'اسمها\s+([^،,.]+)', raw) or re.search(r'اسم العقار\s+([^،,.]+)', raw)
    if m: name = m.group(1).strip()
    elif type_word:
        m = re.search(type_word + r'\s+([^،,.]+)', raw)
        if m:
            cand = m.group(1).strip()
            cand = re.split(r'\s+(في|بـ|بحي|بالرياض|بجده|بمكه|وال|وانا|واني)', cand)[0].strip()
            name = cand
    contract_desc = 'مستثمر' if ptype == 'مستثمر' else ''
    errors=[]
    if not name: errors.append('اسم العقار')
    if not location: errors.append('الموقع')
    return {'ok': len(errors)==0,'name':name,'location':location,'type':ptype,'investor_rent':investor_rent,'contract_desc':contract_desc,'errors':errors}

def parse_units_text(user_id:str, text:str):
    prop = find_property_for_text(user_id, text)
    errors=[]
    if not prop: errors.append('تحديد العقار بالاسم')
    patterns = [('شقة','شقق?'),('استديو','استديو(?:هات)?'),('غرفة','غرف(?:ه)?'),('شاليه','شاليهات?'),('استراحة','استراحات?'),('فله','فلل?|فله')]
    items=[]
    for unit_type, pat in patterns:
        for m in re.finditer(r'(\d+|واحد|واحدة|اثنين|ثنين|ثنتين|ثلاثة|ثلاث|اربعة|أربعة|خمسة|ستة|سبعة|ثمانية|تسعة|عشرة)\s+'+pat, text):
            count = ar_num(m.group(1))
            if count:
                items.append({'unit_type':unit_type,'count':count})
    if not items: errors.append('عدد ونوع الوحدات مثل 3 شقق و2 استديو')
    start_from = None
    total_new_units = sum(i['count'] for i in items)
    if prop:
        units = sb_select('units', {'user_id': f'eq.{user_id}', 'property_id': f"eq.{prop['id']}"})
        nums = []
        for u in units:
            try: nums.append(int(str(u.get('unit_num')).strip()))
            except Exception: pass
        start_from = (max(nums)+1) if nums else 1
    return {'ok': len(errors)==0,'property_id': prop['id'] if prop else None,'property_name': prop.get('name') if prop else '', 'items':items, 'start_from':start_from, 'total_new_units':total_new_units, 'errors':errors}

def parse_tenant_text(user_id:str, text:str):
    raw = text.strip()
    nt = normalize_text(raw)
    prop = find_property_for_text(user_id, raw)
    units = sb_select('units', {'user_id': f'eq.{user_id}', 'property_id': f"eq.{prop['id']}"}) if prop else []
    unit_type = ''
    for w in ['شقة','استديو','غرفة','شاليه','استراحة','فله']:
        if normalize_text(w) in nt:
            unit_type = w
            break
    unit_num = None
    m = re.search(r'(?:شقه|استديو|غرفه|شاليه|استراحه|فله)?\s*(\d+)', nt)
    if m: unit_num = m.group(1)
    # if no explicit num and only one vacant unit, use it
    if prop and not unit_num and units:
        vacant = [u for u in units if not u.get('tenant_name')]
        if len(vacant) == 1: unit_num = str(vacant[0].get('unit_num'))

    name = ''
    m = re.search(r'على\s+([^\d،,.]+)', raw) or re.search(r'ل(?:ل|ـ)?\s*([^\d،,.]+)', raw)
    if m:
        name = re.split(r'\s+(ب|بمبلغ|لمده|لمدة|شهر|يوم|سنه|سنة|في)', m.group(1).strip())[0].strip()
    rent = 0
    m = re.search(r'(\d+)\s*ريال', raw) or re.search(r'ب\s*(\d+)', nt)
    if m: rent = int(m.group(1))
    period='شهر'
    if 'يوم' in nt: period='يوم'
    elif 'سنه' in nt or 'سنة' in nt or 'سنوي' in nt: period='سنة'
    period_count = 1
    m = re.search(r'(\d+)\s*(يوم|شهر|سنه|سنة)', nt)
    if m:
        period_count = int(m.group(1))
        if 'يوم' in m.group(2): period='يوم'
        elif 'شهر' in m.group(2): period='شهر'
        else: period='سنة'
    start_date = datetime.now().date().isoformat()
    m = re.search(r'من\s+(اليوم)', nt)
    if m: start_date = datetime.now().date().isoformat()
    co = '14:00'
    if '12' in nt and ('خروج' in nt or 'تسليم' in nt): co='12:00'
    elif '16' in nt and ('خروج' in nt or 'تسليم' in nt): co='16:00'
    end_date = calc_end_date(start_date, period, period_count)
    errors=[]
    if not prop: errors.append('تحديد العقار')
    if not unit_num: errors.append('رقم الوحدة')
    if not name: errors.append('اسم المستأجر')
    if not rent: errors.append('مبلغ الإيجار')
    # validate unit exists if possible
    if prop and unit_num:
        matched = [u for u in units if str(u.get('unit_num')) == str(unit_num)]
        if not matched: errors.append('الوحدة غير موجودة في العقار المحدد')
        elif matched[0].get('tenant_name'): errors.append('الوحدة مشغولة حالياً')
        elif not unit_type: unit_type = matched[0].get('unit_type','')
    return {'ok': len(errors)==0, 'property_id': prop['id'] if prop else None, 'property_name': prop.get('name') if prop else '', 'unit_type': unit_type, 'unit_num': unit_num, 'name': name, 'rent': rent, 'period': period, 'period_count': period_count, 'start_date': start_date, 'end_date': end_date, 'checkout_time': co, 'errors': errors}

@app.route('/api/smart/property/preview', methods=['POST'])
@require_auth
def smart_property_preview(user):
    d = request.json or {}
    return jsonify(parse_property_text(d.get('text','')))

@app.route('/api/smart/units/preview', methods=['POST'])
@require_auth
def smart_units_preview(user):
    d = request.json or {}
    return jsonify(parse_units_text(user['user_id'], d.get('text','')))

@app.route('/api/smart/units/create', methods=['POST'])
@require_auth
@require_write
def smart_units_create(user):
    d = request.json or {}
    prop_id = d.get('property_id')
    items = d.get('items') or []
    if not prop_id or not items:
        return jsonify({'error':'بيانات ناقصة'}), 400
    units = sb_select('units', {'user_id': f"eq.{user['user_id']}", 'property_id': f'eq.{prop_id}'})
    nums=[]
    for u in units:
        try: nums.append(int(str(u.get('unit_num')).strip()))
        except Exception: pass
    ...
    for item in items:
        ...
        unit_type = item.get('unit_type') or 'شقة'
        for _ in range(count):
            created.append(sb_insert('units', {'user_id': str(user['user_id']), 'property_id': prop_id, 'unit_num': str(next_num), 'unit_type': unit_type}))
            next_num += 1
...
def smart_tenant_preview(user):
    d = request.json or {}
    return jsonify(parse_tenant_text(user['user_id'], d.get('text','')))
